...
name_arr = []
for name in range(n):
    name_arr.append(str(input("Enter employee name: ")))
task_arr = []
for name in range(n):
    task_arr.append(str(input("Enter a task name: ")))
matrix_ef = []
for i in range(n):
    matrix_ef.append([0]*n)
for row in range(0,n):
    for col in range(0,n):
        matrix_ef[row][col] = int(input(f"Enter the performance level of {name_arr[row]} on {task_arr[col]}: "))
...
print(res)
for i in range(0,n):
    print (f"{task_arr[xy_arr[i]]} is solved by employee {name_arr[i]}")